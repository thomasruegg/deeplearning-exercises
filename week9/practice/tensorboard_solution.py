...
valid_log_dir = "logs/1_run/valid"
...
for epoch in range(EPOCHS):
    ...
    # Write histograms to TensorBoard
    ...
    with image_writer.as_default():
        img_arr = tf.concat(mislabeled_images_list, axis=0)
        image = tf.concat(
            (
                tf.reshape(img_arr[0:3, :], (-1, 84, 28, 1)),
                tf.reshape(img_arr[3:6, :], (-1, 84, 28, 1)),
                tf.reshape(img_arr[6:9, :], (-1, 84, 28, 1)),
                tf.reshape(img_arr[9:12, :], (-1, 84, 28, 1)),
            ),
            axis=2,
        )
        tf.summary.image(
